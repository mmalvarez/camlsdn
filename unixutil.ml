...
struct
  ...
  let treat_with_chan_handler handler sock (client_sock, client_addr as client) =
    ...
    let chan_handler sub_handler = 
      (fun sock -> 
        let (in_chan, out_chan) = open_connection sock in
        sub_handler in_chan out_chan);

    in fork_treatment sock (chan_handler handler);;
