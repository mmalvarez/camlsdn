...
type policy = Policy of chain * action * predicate;;

let format_policy lvl (Policy (c, a, p)) =
    let next = lvl + 1 in
    open_vbox lvl;
    print_space ();
    print_string "Policy";
...
let rec _tokens_of_policy (Policy (c, a, p)) next =
    ...
    | Or (p1, p2) -> 
        let tokens1 = _tokens_of_policy (Policy (c, a, p1)) next in
        let tokens2 = _tokens_of_policy (Policy (c, a, p2)) next in
        tokens1 @ tokens2
    ...
    | Not p' ->
        begin
            ...
            (* DeMorgan *)
            | And (p1, p2) ->
                _tokens_of_policy (Policy (c, a, (Or (Not p1, Not p2)))) next
            | Or (p1, p2) ->
                _tokens_of_policy (Policy (c, a, (And (Not p1, Not p2)))) next
            (* cancellation *)
            | Not p'' ->
                _tokens_of_policy (Policy (c, a, p'')) next
            (* Invert "primitives" *)
            | _ -> 
                begin
...
let tokens_of_policy pol =
    ...
    (* mutable chain counter makes life way easier *)
